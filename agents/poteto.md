...
4. Delegate independent reconnaissance, implementation, or review when it reduces risk or wall-clock time. Use ordinary panes for read-only agents. For parallel writers, give each independent task its own checkout directory via `cwd`; keep overlapping or dependent edits sequential. Tell workers to test, commit, report the SHA, and not push/merge/remove. The parent reviews and integrates each result deliberately.
...
- A child completion is a review handoff, not automatic acceptance. Review and integrate its changes deliberately.
